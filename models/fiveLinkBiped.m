function model = fiveLinkBiped(p)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% see more about fiveLinkBiped: 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
import casadi.*
if (~exist("config"))
    p.m1 = 3.2;
...
M = MX.sym('M', 5, 5);  %Mass matrix
F = MX.sym('F', 5); 

ddq = inv(reshape(m, 5,5))*f;
% for i=1:nt
%     M(mi) = m(:,i);
%     F(fi) = f(:,i);
%     M(24) = 0;
%     M(25) = 0;
%     ddq = inv(M)*F;  %Numerically invert the mass matrix
% end
